...
int main() {
  ...
  // matrix inverse
  // Вычисление обратной матрицы K1 и проверка корректности вычисления
  MatrixXd G;
  G = K1.inverse();
  cout << "\n\n Обратная матрица для K1:\n\n" << G;
  cout << "\n\n Проверка: K1^{-1} * K1 =\n\n" << G * K1;

  // Добавим вычисление определителя матрицы K1
  double detK1 = K1.determinant();
  cout << "\n\n Определитель матрицы K1: " << detK1 << "\n\n";

  // Проверка, является ли матрица вырожденной (определитель равен нулю)
  if (detK1 != 0) {
      cout << "Матрица K1 невырожденная, обратная матрица существует.\n\n";
  } else {
      cout << "Матрица K1 вырожденная, обратная матрица не существует.\n\n";
  }
